...
object ForminConflictResolver extends ConflictResolver[ForminConfig] {
  ...
  override def resolveConflict(current: GridPart, incoming: SmellingCell)(implicit config: ForminConfig): GridPart = {
    (current, incoming) match {
      ...
      case (AlgaeCell(currentSmell, _), ForaminiferaCell(energy, incomingSmell, lifespan)) =>
        ForaminiferaCell(energy + config.algaeEnergeticCapacity, incomingSmell + currentSmell, lifespan)
      case (ForaminiferaCell(energy, incomingSmell, lifespan), AlgaeCell(currentSmell, _)) =>
        ForaminiferaCell(energy + config.algaeEnergeticCapacity, incomingSmell + currentSmell, lifespan)
      case (AlgaeCell(currentSmell, lifespan), AlgaeCell(incomingSmell, incominglifespan)) =>
        AlgaeCell(currentSmell + incomingSmell, math.max(lifespan, incominglifespan))
      case (ForaminiferaCell(currentEnergy, currentSmell, lifespan), ForaminiferaCell(incomingEnergy, incomingSmell, incominglifespan)) =>
        ForaminiferaCell(currentEnergy + incomingEnergy, currentSmell + incomingSmell, math.max(lifespan, incominglifespan))
