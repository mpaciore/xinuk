...
object DirtAccessible {
  ...
  def unapply(arg: StudentCell)(implicit config: SchoolConfig): DirtAccessible[DirtCell] =
    new DirtAccessible[DirtCell] {
      override def withDirt(energy: Energy, lifespan: Long): DirtCell = DirtCell(energy + config.algaeEnergeticCapacity, arg.smellWith(config.cleanerInitialSignal.toSignalVector), lifespan, config.cleanerInitialSignalIndex)
    }

  def unapply(arg: EmptyCell)(implicit config: SchoolConfig): DirtAccessible[DirtCell] =
    new DirtAccessible[DirtCell] {
      override def withDirt(energy: Energy, lifespan: Long): DirtCell = DirtCell(energy, arg.smellWith(config.cleanerInitialSignal.toSignalVector), lifespan, config.cleanerInitialSignalIndex)
    }

  def unapply(arg: BufferCell)(implicit config: SchoolConfig): DirtAccessible[BufferCell] =
    new DirtAccessible[BufferCell] {
      override def withDirt(energy: Energy, lifespan: Long): BufferCell = BufferCell(DirtCell(energy, arg.smellWith(config.cleanerInitialSignal.toSignalVector), lifespan, config.cleanerInitialSignalIndex))
    }
