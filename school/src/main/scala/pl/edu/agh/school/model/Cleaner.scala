...
object CleanerAccessible {
  ...
  def unapply(arg: DirtCell)(implicit config: SchoolConfig): CleanerAccessible[CleanerCell] =
    (energy: Energy, lifespan: Long) => CleanerCell(energy, arg.smellWith(config.cleanerInitialSignal.toSignalVector), lifespan, config.cleanerInitialSignalIndex)
  ...
  def unapply(arg: BufferCell)(implicit config: SchoolConfig): CleanerAccessible[BufferCell] =
    (energy: Energy, lifespan: Long) => BufferCell(CleanerCell(energy, arg.smellWith(config.cleanerInitialSignal.toSignalVector), lifespan, config.cleanerInitialSignalIndex))
