...
class WorkerActor private(id: WorkerId)(implicit config: ForminConfig) extends Actor with ActorLogging with Stash {
  ...
  private def makeMoves(iteration: Long): Metrics = {
    ...
    grid = newGrid
    Metrics(foraminiferaCount, algaeCount)
  }
  ...
  def stopped: Receive = {
    val specific: Receive = {
      ...
      case StartIteration(1) =>
        ...
        propagateSignal()
        notifyListeners(1, grid, Metrics(foraminiferaCount, algaeCount))
        unstashAll()
...
final case class Metrics(foraminiferaCount: Long, algaeCount: Long)
