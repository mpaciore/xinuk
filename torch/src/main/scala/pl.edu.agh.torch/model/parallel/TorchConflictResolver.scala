...
object TorchConflictResolver extends ConflictResolver[TorchConfig] {
  ...
  override def resolveConflict(current: GridPart, incoming: SmellingCell)(implicit config: TorchConfig): (GridPart, TorchMetrics) = {
    (current, incoming) match {
      ...
      case (EscapeCell(currentSmell), HumanCell(_, _, _, _)) =>
      ...
      case (FireCell(currentSmell), HumanCell(incomingSmell, _, _, _)) =>
        (FireCell(currentSmell + incomingSmell), TorchMetrics(0, 0, 0, 1, 0))
      case (HumanCell(currentSmell, _, _, _), FireCell(incomingSmell)) =>
        (FireCell(currentSmell + incomingSmell), TorchMetrics(0, 0, 0, 1, 0))
      case (HumanCell(currentSmell, currentCrowd, currentSpeed, pursuedSignalIndex), another@HumanCell(incomingSmell, incomingCrowd, _, _)) =>
        (HumanCell(currentSmell + incomingSmell, currentCrowd ++ incomingCrowd ++ List(another), currentSpeed, pursuedSignalIndex), TorchMetrics.empty())
