...
final case class EscapeCell(signal: SmellMap) extends GridPart {
  override type Self = EscapeCell

  override def withSmell(smell: SmellMap): EscapeCell = copy(signal = smell)
}
...
object EscapeAccessible {
  ...
  def unapply(arg: EmptyCell)(implicit config: TorchConfig): EscapeAccessible[EscapeCell] =
    new EscapeAccessible[EscapeCell] {
      override def withEscape(): EscapeCell = EscapeCell(arg.signalWith(config.escapeInitialSignal))
    }
}
